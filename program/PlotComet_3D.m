function [cfig , M_out] = PlotComet_3D(x_data, y_data, z_data, t_data, varargin)
...
for n = 1:1:length(x_data)
    ...
    plot3(x_data(n_start:n_stop),...
        ...
        z_data(n_start:n_stop),tailFormat);
    plot3(x_data(n_stop), y_data(n_stop), z_data(n_stop),headFormat);
    text(x_data(n),y_data(n),z_data(n),['(' num2str(x_data(n)) ',' num2str(y_data(n)) ',' num2str(t_data(n)) ')']);
    %
    
    drawnow;
    M(n) = getframe(cFigure);
    
    % Creation of a Video object
     %VR = VideoRecorder('Filename',     'Video_TTCINV_M1_vs_THWINV_M1',...
     %                   'Fileformat',   'MPEG-4',...
     %                   'Figure',       cFigure,...
     %                   'Period',       0.05,...
     %                   'BusyMode',     'error');
...
%m = length(x_data);
%for i = 1:m
% set(headFormat,'xdata',x_data(i),'ydata',y_data(i),'zdata',z_data(i))
% set(tailFormat,'xdata',x_data(1:i),'ydata',y_data(1:i),'zdata',z_data(1:i))
% drawnow
% M(iter) = getframe;
% iter = iter+1;
%end
